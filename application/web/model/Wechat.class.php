...
class Wechat
{
	...
	const MENU_CREATE_URL = '/menu/create?';//生成菜单
	const MENU_GET_URL = '/menu/get?';//获取菜单
	const MENU_DELETE_URL = '/menu/delete?';//删除菜单
	...
	{
		\Think\Log::write('$options--options-:'.print_r($options,1));
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	private function http_get($url){
		$oCurl = curl_init();
		if(stripos($url,"https://")!==FALSE){  //url中包含了http
		...
		$sContent = curl_exec($oCurl);  //// 抓取URL并把它传递给浏览器  成功返回true 失败返回flase
		$aStatus = curl_getinfo($oCurl);   //获取一个cURL连接资源句柄的信息
		curl_close($oCurl);  //
		if(intval($aStatus["http_code"])==200){   // 获取变量的整数值
			return $sContent;
		}else{
			return false;
		}
	...
	protected function setCache($cachename,$value,$expired){
		$mem=new MyMemcache();
		$mem->set($cachename,$value,$expired);
		return true;
	}
...
//			\Think\Log::write('result-access_token1:'.print_r($token,1));
		    return $this->access_token;
		}
//		$authname = 'wechat_access_token'.$appid;
//		if ($rs = $this->getCache($authname))  {
//			$this->access_token = $rs;
//			\Think\Log::write('result-access_token2:'.print_r($rs,1));
//			return $rs;
//		}

		$result = $this->http_get(self::API_URL_PREFIX.self::AUTH_URL.'appid='.$appid.'&secret='.$appsecret);
		\Think\Log::write('result-access_token:'.print_r($result,1));
		if ($result)
		{
			$json = json_decode($result,true);
			\Think\Log::write('wechat-class-checkAuth---:'.print_r($json,1));
	...
	public function getJsCardTicket($appid='',$api_ticket=''){
		iecho($this->access_token);
		iecho($this->checkAuth());
		exit;

		if (!$this->access_token && !$this->checkAuth()) iecho('dss'); return false;
		echo 'no accesstoken';
		if (!$appid) $appid = $this->appid;
	...
	public function createMenu($data){
		...
		\Think\Log::write('$data--json_decode'.print_r($data,1));
		if (!$this->access_token && !$this->checkAuth()) return false;
		\Think\Log::write('$this->access_token:1'.print_r($this->access_token,1));
		//$this->checkAuth();
		\Think\Log::write('$this->access_token:2'.print_r($this->access_token,1));
		...
		\Think\Log::write('createMenu--$result:'.print_r($result,1));
		if ($result)
		{
			$json = json_decode($result,true);
			\Think\Log::write('createMenu--$json---:'.print_r($json,1));
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
				$this->errMsg = $json['errmsg'];
				return false;
	...
	public function uploadImg($data){
		...
		$data['media']     =  '@C:\\Users\\liqing\\Documents\WeChat Files\\liqing164428\\Attachment\\0d96667f0dc4281748210801fde3b4a0_t.jpg';
	...
	public function getOauthAccessToken(){
		\Think\Log::write('2420--code--getOauthAccessToken-:'.$_GET['code']);
		$code = isset($_GET['code'])?$_GET['code']:'';
		if (!$code) return false;
		$result = $this->http_get(self::API_BASE_URL_PREFIX.self::OAUTH_TOKEN_URL.'appid='.$this->appid.'&secret='.$this->appsecret.'&code='.$code.'&grant_type=authorization_code');\Think\Log::write('2420--code--getOauthAccessToken-$result---:'.$result);
		\Think\Log::write('get_open_return:'.print_r($result,1));
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
	...
	public function sendTemplateMessage($data){
		if (!$this->access_token && !$this->checkAuth()) return false;
		\Think\Log::write('wechat---$this->access_token--'.print_r($this->access_token,1));
		$result = $this->http_post(self::API_URL_PREFIX.self::TEMPLATE_SEND_URL.'access_token='.$this->access_token,$data);
		if($result){
			$json = json_decode($result,true);
			\Think\Log::write('wechat---sendTemplateMessage--'.print_r($json,1));
			//iecho($json);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
				$this->errMsg = $json['errmsg'];
